...
10. Select multiple files and confirm the delete warning clearly says deletion is immediate, permanent, and cannot be undone. Delete them, confirm the dialog remains open with disabled controls while requests run, and verify each successful row disappears after refetch. Block one DELETE request to simulate a partial failure and confirm successful rows clear while the failed row remains selected and is reported for retry.
11. For a successfully deleted file, confirm its owner download endpoint now returns `404`, its DynamoDB item is absent, and its exact S3 object no longer exists. Using a second signed-in test user, attempt to delete another remaining first-user file ID and confirm the response is the same `404` as a missing ID and that both metadata and bytes remain. Request `DELETE /api/files/<id>` signed out and confirm API Gateway returns `401` without invoking the delete Lambda.
